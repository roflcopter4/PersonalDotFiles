...
function xsource () {
    ...
    if [[ "$1" == '-e' ]]; then
        ...
        while (( ${#argv} > 0 )) ; do
            if [[ -r "$1" ]]; then
                source "$1"
            elif [[ -r ""$HOME"/.$(basename "$1")" ]]; then
                source ""$HOME"/.$(basename "$1")"
                #printf "Unable to source '%s', sourced '%s' instead\n" "$1" ""$HOME"/.$(basename "$1")"
            else
                #printf "Unable to source '%s'\n" "$1"
            fi
            shift
        done
    fi
...
if zrcautoload compinit ; then
    compinit -i -d ${COMPDUMPFILE} || print 'Notice: no compinit available :('
else

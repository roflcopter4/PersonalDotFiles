...
do_highlight() {
    #msg "LC -> ${2:-${LESSCOLORIZER}}"
    if [[ $# -eq 2 ]]; then
        $2 "$1"
    elif [[ "$LESSCOLORIZER" ]]; then
        $LESSCOLORIZER "$1"
    elif cmd_exist "code2color"; then
        code2color $PPID ${in_file:+"$in_file"} $lang "$1"
    fi
}
...
isfinal() {
    ...
    if [[ -n "$LESSH" ]]; then
        #msg 'lessh mode'
        #local NN=0
        #for ii in "$@"; do msg "var ${NN} = \`\`${ii}''"; NN=$((NN + 1)); done
        do_highlight "${2}" "${_LESSCOLORIZER_}"
        return
    fi

    if [[ "${3}" == "${sep}" ]]; then
        msg "Avoiding syntax highlighting."
        cat "$2"
        return
    fi
